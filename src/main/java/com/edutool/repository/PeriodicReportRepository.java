...
public interface PeriodicReportRepository extends JpaRepository<PeriodicReport, Integer> {
    ...
    @Query("SELECT pr FROM PeriodicReport pr WHERE pr.course.courseId = :courseId AND pr.status = 'ACTIVE' ORDER BY pr.reportFromDate DESC")
    List<PeriodicReport> findByCourse_CourseId(@Param("courseId") Integer courseId);
    
    // Lấy tất cả periodic report (chỉ active)
    @Query("SELECT pr FROM PeriodicReport pr WHERE pr.status = 'ACTIVE' ORDER BY pr.createdAt DESC")
    List<PeriodicReport> findAllActive();
    ...
    @Query("SELECT pr FROM PeriodicReport pr " +
           ...
           "ORDER BY pr.reportFromDate DESC")
    List<PeriodicReport> findByCourseAndDateRange(@Param("courseId") Integer courseId,
    ...
    @Query("SELECT pr FROM PeriodicReport pr " +
           ...
           "AND pr.status = 'ACTIVE' " +
           "ORDER BY pr.submitStartAt DESC")
    List<PeriodicReport> findOpenReportsByCourse(@Param("courseId") Integer courseId,
    ...
    @Query("SELECT pr FROM PeriodicReport pr WHERE pr.status = 'INACTIVE' ORDER BY pr.createdAt DESC")
    List<PeriodicReport> findInactive();
}
